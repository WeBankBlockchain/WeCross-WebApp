    ...
    return new Promise((resolve, reject) => {
      startXATransaction(transaction).then(response => {
        if (response.errorCode !== 0) {
          Message.error({ message: '开启事务失败，错误：' + response.message, center: true })
          reject()
        } else {
  ...
  commitTransaction({ commit }, transaction) {
    return new Promise((resolve, reject) => {
      commitXATransaction(transaction).then(response => {
        if (response.errorCode !== 0) {
          Message.error({ message: '提交事务失败，错误：' + response.message, center: true })
          reject()
        } else {
          commit('RESET_STATE')
          resolve()
  ...
  rollbackTransaction({ commit }, transaction) {
    return new Promise((resolve, reject) => {
      rollbackXATransaction(transaction).then(response => {
        if (response.errorCode !== 0) {
          Message.error({ message: '回滚事务失败，错误：' + response.message, center: true })
          reject()
        } else {
          commit('RESET_STATE')
          resolve()
